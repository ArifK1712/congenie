          ...
          {workflows.map((flow, idx) => {
            ...
            return (
              <motion.div
                ...
                className="flex flex-col md:flex-row items-center justify-between p-6 sm:p-8 rounded-3xl border border-slate-200/50 bg-slate-50 gap-6"
              >
                {/* Trigger Block */}
                <div className="flex items-center space-x-4 w-full md:w-1/3">
                  ...
                  <div>
                    ...
                    <h3 className="text-base font-bold text-slate-850">{flow.trigger}</h3>
                  </div>
                </div>

                {/* Connection arrow on larger screens */}
                <div className="hidden md:flex text-slate-350 flex-1 justify-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-1/2">
                  {flow.actions.map((act, actIdx) => {
                    ...
                    return (
                      <div
                        key={actIdx}
                        className="flex-1 glass-card p-4 rounded-2xl border border-slate-200/50 flex items-center space-x-3 hover:border-accent-cyan/30 transition-all duration-300"
                      >
